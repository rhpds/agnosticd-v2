...
In order to deploy a configuration you will need to set up the variables for the configuration. An example config to deploy an OpenShift Cluster is copied into the `agnosticd-v2-vars` directory when running `agd setup`. This file works out of the box - but you can of course modify it to suit your needs. At a minimum you should update your e-mail address under `cloud_tags` and replace the `ssh_authorized_keys` variable with your public key hosted on Github.
